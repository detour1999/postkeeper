...
describe("CLI", () => {
  ...
  test("--version shows version", () => {
    const output = execFileSync("node", [cli, "--version"], { encoding: "utf-8", cwd });
    assert.match(output.trim(), /^\d+\.\d+\.\d+$/);
  });

  test("list shows installed plugins", () => {
    const output = execFileSync("node", [cli, "list"], { encoding: "utf-8", cwd });
    assert.ok(output.includes("instagram"));
  });

  test("init with unknown plugin fails", () => {
    assert.throws(
      () => execFileSync("node", [cli, "init", "nonexistent"], { encoding: "utf-8", stdio: "pipe", cwd }),
      (err) => err.status !== 0
    );
  });
  ...
  test("status with unknown plugin fails", () => {
    assert.throws(
      () => execFileSync("node", [cli, "status", "nonexistent"], { encoding: "utf-8", stdio: "pipe", cwd }),
      (err) => err.status !== 0
    );
  });
